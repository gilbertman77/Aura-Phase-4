...
BLASTEROIDS
Asteroids clone for Python

Original code by Amin Bandali
Modified by Aura Inc
...
class Spaceship(GameObject):
    ...
    def move(self, screen):
        ...
        #move if not at edge of screen
        if self.position[0] < 0:    # or self.postion[0] > screen.get_height()
            self.position[0] = 1
            return
        elif self.position[0] > screen.get_width():
            self.position[0] = screen.get_width()
            return
        if self.position[1] < 0:    # or self.postion[0] > screen.get_height()
            self.position[1] = 1
            return
...
class Pad(GameObject):
    """Resembles a pad"""
    def __init__(self, position):
        super(Pad, self).__init__(position,\
            load_image_convert_alpha('pad.png'))
...
class MyGame(object):
    ...
    def __init__(self):
        ...
        self.bg = load_image_convert_alpha('background.png')
    ...
    def do_welcome(self):
        ...
        self.welcome_asteroids = self.big_font.render("Blasteroids",\
                                                True, (255, 215, 0))
    ...
    def start(self):
        ...
        self.startpad = Pad((self.width//2, 0 +30 ))
        self.endpad = Pad((self.width//2, self.height - 30))
       
        self.missiles = []

        # start the sound track loop
        # BROKEN self.soundtrack.play(-1, 0, 1000)
                ...
                if self.state != MyGame.WELCOME:
                    ...
                    if keys[pygame.K_SPACE]:
                        ...
                        if new_time - self.fire_time > \
                            ...
                            # 0.15s delay between firing  missiles

                            # fire a missile
                            self.spaceship.fire()

                            # play the sound
                            ## BROKEN self.missile_sound.play()
    ...
    def game_over(self):
        ...
        ## BROKEN self.gameover_sound.play()
        ## BROKEN delay = int((self.gameover_sound.get_length()+1)*1000)
        delay = 1000
    ...
    def die(self):
        """Losing a life"""
        ## BROKEN self.soundtrack.stop()
        # play dying sound and wait for it to end before continuing
        self.lives -= 1
        self.counter = 0        
        self.state = MyGame.DYING
        ## BROKEN self.die_sound.play()
        ## BROKEN delay = int((self.die_sound.get_length()+1)*1000)
        delay = 1000
        ...
        if len(self.rocks) > 0:
            ...
            for rock in self.rocks:
                ...
                # 10 rocks on the screen, create a new rock with the same size
                    ...
                    if rock.position[0] < 0 or rock.position[0] > self.width:
                        rock.direction[0] *= -1
                    elif rock.position[1] < 0 or rock.position[1] > self.height:
                        rock.direction[1] *= -1
    ...
    def draw(self):
        ...
        if self.state != MyGame.WELCOME:
            ...
            # draw the pads
            self.startpad.draw_on(self.screen)
            self.endpad.draw_on(self.screen)
            ...
            scores_text = self.medium_font.render("Score " + "0",\
                                                    True, (0, 155, 0))
